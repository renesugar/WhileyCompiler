...
public class StdTypeMangler implements TypeMangler {
	...
	private void writeTypeMangle(Type t, Tuple<Identifier> lifetimes, StringBuilder mangle) {
		...
		case TYPE_intersection:
			writeTypeMangleIntersection((Type.Intersection) t, lifetimes, mangle);
			break;
		default:
	...
	private void writeTypeMangleIntersection(Type.Intersection t, Tuple<Identifier> lifetimes, StringBuilder mangle) {
		mangle.append('i');
		mangle.append(t.size());
		for(int i=0;i!=t.size();++i) {
			writeTypeMangle(t.get(i), lifetimes, mangle);
