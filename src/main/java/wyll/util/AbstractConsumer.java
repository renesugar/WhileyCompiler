...
public abstract class AbstractConsumer<T> {
	...
	public void visitWhileyFile(WyllFile wf, T data) {
	...
	public void visitAssign(Stmt.Assign stmt, T data) {
		visitLVal(stmt.getLeftHandSide(), data);
		visitExpression(stmt.getRightHandSide(), data);
	}

	public void visitLVal(LVal lval, T data) {
		visitExpression(lval, data);
	}
	...
	public void visitForEach(Stmt.ForEach stmt, T data) {
		visitVariable(stmt.getVariable(), data);
	...
	public void visitIfElse(Stmt.IfElse stmt, T data) {
		...
		for(int i=0;i!=branches.size();++i) {
			visitExpression(branches.get(i).getFirst(), data);
		}
		if(stmt.hasDefaultBranch()) {
			visitBlock(stmt.getDefaultBranch(), data);
		}
	}
	...
	public void visitExpression(Expr expr, T data) {
		...
		case EXPR_indirectinvoke:
			...
			break;
		case EXPR_lambda:
			visitLambda((Expr.Lambda) expr, data);
			break;
	...
	public void visitUnaryOperator(Expr.UnaryOperator expr, T data) {
		...
		case EXPR_unionaccess:
			...
			break;
		case EXPR_unionenter:
			visitUnionEnter((Expr.UnionEnter) expr, data);
			break;
		case EXPR_unionleave:
			visitUnionLeave((Expr.UnionLeave) expr, data);
			break;
		case EXPR_clone:
			visitClone((Expr.Clone) expr, data);
			break;
		case EXPR_integernegation:
	...
	public void visitClone(Expr.Clone expr, T data) {
		visitExpression(expr.getOperand(), data);
	}
	...
	public void visitLambda(Expr.Lambda expr, T data) {
		visitVariables(expr.getParameters(), data);
		visitVariables(expr.getReturns(), data);
		visitExpression(expr.getBody(), data);
	}
	...
	public void visitUnionEnter(Expr.UnionEnter expr, T data) {
		visitExpression(expr.getOperand(), data);
	}

	public void visitUnionLeave(Expr.UnionLeave expr, T data) {
		visitExpression(expr.getOperand(), data);
	}
