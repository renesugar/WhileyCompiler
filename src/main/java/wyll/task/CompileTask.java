...
public class CompileTask {
	...
	private final TypeMangler mangler = new StdTypeMangler();
	private boolean debug = true;

	public CompileTask(TypeSystem typeSystem) {
	...
	public WyllFile.Stmt translateSwitch(Stmt.Switch stmt, Context context) {
		...
		for (int i = 0; i != cases.size(); ++i) {
			ncases[i] = translateCase((Stmt.Case) cases.get(i), type, context);
			if(ncases[i] == null) {
	...
	public WyllFile.Expr translateInvoke(Expr.Invoke expr, Type target, Context context) {
		...
		WyllFile.Type.Method type = (WyllFile.Type.Method) translateCallable(expr.getSignature());
